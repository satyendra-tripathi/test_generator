...
from nlp_utils import tag_difficulty, extract_keywords, generate_questions
import json
import random
import sqlite3
...
def user_dashboard():
    ...
    cursor = execute_query("SELECT * FROM attempts WHERE user_id=?", (user['id'],))
    attempts = cursor.fetchall()
...
def generate_test():
    ...
    cursor = execute_query("SELECT * FROM questions WHERE subject=?", (subject,))
    questions = cursor.fetchall()
    ...
    session['current_test'] = formatted
    return redirect(url_for('test_page'))
...
@login_required
def test_page():
    return render_template('test_page.html', questions=session.get('current_test'))


@app.route('/submit_test', methods=['POST'])
@login_required
def submit_test():
    questions = session.get('current_test')
...
@admin_required
def admin_dashboard():
    cursor = execute_query("SELECT * FROM questions")
    questions = cursor.fetchall()
    return render_template('admin_dashboard.html', questions=questions, attempts=[])
...
def add_question():
    ...
    execute_query(
        "INSERT INTO questions (question, options, answer, subject, difficulty, tags) VALUES (?,?,?,?,?,?)",
        (q, json.dumps(options), ans, sub, tag_difficulty(q), ",".join(extract_keywords(q))),
        commit=True
    )
